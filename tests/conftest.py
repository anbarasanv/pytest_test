...
def sample_input_data():
    """Fixture: Sample input data for testing predictions."""
    return {
...
def sample_input_df(sample_input_data):
    """Fixture: Convert sample input data to a pandas DataFrame."""
    return pd.DataFrame(sample_input_data)


@pytest.fixture
def raw_training_data():
    """Fixture: Small subset of the training data for tests."""
    return pd.DataFrame({
...
def pipeline_trained():
    """Fixture: Load the trained model pipeline."""
...
def prediction_input():
    """Fixture: Minimal input example for model prediction testing."""
    return pd.DataFrame({
        "dteday": ["2012-11-05", "2011-07-13"],
        "season": ["winter", "fall"],
        "hr": ["6am", "4am"],
